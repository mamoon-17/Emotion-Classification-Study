...
class SentimentExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        sentiment_scores = np.array([TextBlob(x).sentiment.polarity for x in X])
        return sentiment_scores.reshape(-1, 1)
...
pipeline = Pipeline([
    ...
    ('logreg', LogisticRegression(max_iter=4000, class_weight=class_weights))
])

params = {
    'features__tfidf__max_features': [30000],
    'features__tfidf__ngram_range': [(1,2), (1,3)],
    'features__tfidf__analyzer': ['word', 'char_wb'],
    'features__tfidf__min_df': [2, 3],
    'features__tfidf__max_df': [0.8, 0.9],
