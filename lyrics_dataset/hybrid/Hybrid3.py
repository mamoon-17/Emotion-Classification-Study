...
# You can switch back to "distilroberta-base" if RAM is tight
MODEL_NAME     = "roberta-base"
MAX_LENGTH     = 256        # a bit longer context
EMB_BATCH_SIZE = 8          # smaller batch because roberta-base is heavier
SEED           = 42
...
def main():
    ...
    # ------------------ Optional small validation split (no early stopping, just monitoring) --
    ...
    # ------------------ XGBoost classifier (tuned a bit) ------------------
    print("\n================ XGBoost (Hybrid LLM + Classical, tuned) ================")
    xgb_clf = XGBClassifier(
        objective="multi:softprob",
        num_class=len(classes_int),

        # Smaller LR + more trees; some regularisation
        n_estimators=400,
        learning_rate=0.04,
        max_depth=8,
        min_child_weight=3,
    ...
    t1 = time.time()
    # NOTE: no early_stopping_rounds here because the Colab xgboost version
    # doesn't support it for XGBClassifier.
    xgb_clf.fit(
        X_tr,
        y_tr,
    ...
    evaluate_model(
        "Hybrid XGBoost (roberta-base embeddings, tuned)",
...
# ======================================================================
# 🏁 Evaluating: Hybrid XGBoost (roberta-base embeddings, tuned)
# ======================================================================

# Accuracy: 0.7947
# Exact Match (EM): 0.7947
# Macro F1: 0.7944
...
#         calm       0.97      0.94      0.95       610
#        happy       0.62      0.62      0.62       610
