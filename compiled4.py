...
# ========== PIPELINE WITH SCALED EXTRA FEATURES ==========
pipeline = Pipeline([
    ('features', FeatureUnion([
        ('tfidf', TfidfVectorizer(sublinear_tf=True, max_features=20000, ngram_range=(1,3))),
        ('extras', Pipeline([
            ('feat', ExtraFeatures()),
            ('scaler', StandardScaler())  # Scale numeric features for convergence
        ]))
    ])),
    ('logreg', LogisticRegression(max_iter=5000, solver='saga', class_weight=class_weights))
])
