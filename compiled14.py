...
def extract_meta_features(text: str) -> pd.Series:
    ...
    sentiment_strength = abs(sentiment_val)
    ...
    base = {
        ...
        'elongated': elongated,
        'negations': negations,
        'negation_density': negations / (word_count + 1),
...
def build_pipeline(class_weights: dict) -> Pipeline:
    ...
    preprocess = ColumnTransformer([
        ...
        ('num', Pipeline([
            ...
            'sentiment', 'sentiment_strength', 'subjectivity',
            'punctuation_density', 'elongated', 'negations', 'negation_density',
            'uppercase_ratio', 'joy', 'sadness', 'anger', 'fear', 'trust',
...
def main():
    ...
    base_pipeline = build_pipeline(class_weights)
    ...
    t1 = time.time()
    print(f"✅ Training + tuning complete in {round((t1 - t0)/60, 2)} min")
    ...
    plt.title('Optimized Logistic Regression (Feature-Enriched)')
    plt.show()
...
# ✅ Training + tuning complete in 44.31 min
# Best params: {'logreg__penalty': 'l2', 'logreg__l1_ratio': 0.2, 'logreg__C': np.float64(4.6415888336127775)}
...
#         calm       0.95      0.98      0.96       610
#        happy       0.62      0.58      0.60       610
#          sad       0.62      0.61      0.61       610
